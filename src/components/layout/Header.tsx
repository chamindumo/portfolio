  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 h-[var(--header-height)] border-b border-[var(--border-primary)] bg-[var(--background-primary)]/90 backdrop-blur-md transition-colors duration-300">
      <div className="max-w-[var(--grid-max-width)] mx-auto h-full px-3 md:px-6 flex items-center justify-between relative">
        ...
        {/* Left: Meaningful Menu Trigger + Direct Nav Links */}
        <div className="flex items-center gap-3 lg:gap-5">
          {/* Clear [ MENU ] Button */}
          <button
            onClick={onOpenMenu}
            className="relative h-9 px-3 flex items-center gap-2 font-mono text-xs font-bold tracking-wider text-[var(--text-primary)] hover:text-[var(--accent-primary)] bg-[var(--background-secondary)] hover:bg-[var(--background-tertiary)] transition-all border border-[var(--border-primary)] group cursor-pointer"
          ...
          {/* Quick Direct Desktop Navigation Links */}
          <nav className="hidden lg:flex items-center gap-1 font-mono text-xs" aria-label="Quick Nav">
            {navLinks.map((link) => {
          ...
          {/* Section Indicator & Telemetry (Medium screens) */}
          <div className="hidden md:flex lg:hidden items-center gap-2 font-mono text-[11px] uppercase tracking-widest text-[var(--text-quaternary)]">
            <span className="w-1.5 h-1.5 rounded-full bg-[var(--accent-primary)] animate-pulse" />
            <span className="text-[var(--text-primary)] font-bold">{activeSection.toUpperCase()}</span>
            <span className="opacity-40">|</span>
            <span className="text-[var(--accent-primary)] font-mono">{String(scrollProgress).padStart(2, '0')}%</span>
          </div>
        </div>

        {/* Center: Brand Identity & Avatar */}
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center">
          <a
            href="#hero"
            className="flex items-center gap-2 tracking-[0.15em] font-sans font-extrabold text-sm md:text-base text-[var(--text-primary)] hover:text-[var(--accent-primary)] transition-colors select-none group"
          >
            <img
              src="/chamindu.jpg"
              alt="Chamindu Moramudali"
              className="w-5 h-5 rounded-full object-cover border border-[var(--accent-primary)]/60 group-hover:scale-105 transition-transform"
              onError={(e) => {
        ...
        {/* Right: Meaningful Location & Crystal-Clear Theme Switch */}
        <div className="flex items-center gap-2.5 sm:gap-3">
          {/* Meaningful Location / Colombo, Sri Lanka Badge (Replaced old EN/AU) */}
          ...
          {/* Crystal-Clear, High-Contrast AM / PM Theme Switch Button */}
          <button
            onClick={onToggleTheme}
            className="relative h-9 px-3 flex items-center gap-2 font-mono text-xs font-bold border border-[var(--border-primary)] bg-[var(--background-secondary)] hover:border-[var(--accent-primary)] text-[var(--text-primary)] hover:text-[var(--accent-primary)] transition-all cursor-pointer select-none group"
