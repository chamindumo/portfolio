...
export const RealSpaceRocket: React.FC<RealSpaceRocketProps> = ({ isDark = true }) => {
  ...
  useEffect(() => {
    ...
    const ambientLight = new THREE.AmbientLight(isDark ? 0x242832 : 0xf1f5f9, isDark ? 1.4 : 2.0);
    ...
    // Cyan Rim Light (Accents the aerospace silhouette)
    const cyanRim = new THREE.DirectionalLight(0x00d8ff, 2.4);
    cyanRim.position.set(-8, 2, -4);
    scene.add(cyanRim);

    // Engine Exhaust Backlight
    const engineGlow = new THREE.PointLight(0x00e5ff, 3.5, 10);
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center pointer-events-auto select-none">
      ...
      <div className="absolute inset-x-2 bottom-1 flex items-center justify-between pointer-events-none font-mono text-[10px] text-[var(--text-tertiary)] px-3 py-1 bg-[#101217]/70 backdrop-blur-xs border-t border-[var(--border-secondary)]">
        <div className="flex items-center gap-2">
          <span className="w-1.5 h-1.5 rounded-full bg-[#00d8ff] animate-ping" />
          <span className="text-[#00d8ff] font-bold">{telemetry.status}</span>
        </div>
        <div className="flex items-center gap-3">
          <span>ALT: <strong className="text-[var(--text-primary)]">{telemetry.altitude}</strong></span>
          <span>THRUST: <strong className="text-[#00d8ff]">{telemetry.thrust}</strong></span>
